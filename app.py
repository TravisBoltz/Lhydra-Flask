    ...
    def _initialize_system(self):
        ...
        try:
            ...
            self.recommender = RecommendationGenerator(
                str(self.model_path), self.catalog_data, str(self.encoder_path)
            )
    ...
    def _prepare_user_data(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            user_data = {
                ...
                "main_genre": (
                    request_data["favourite_genres"][0]
                    if request_data["favourite_genres"]
                    else "Other"
                ),
    ...
    def get_recommendations(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            recommendations = self.recommender.generate_recommendations(
                user_data, n_recommendations=10
            )
                ...
                return jsonify(
                    {
                        ...
                        "catalog_size": (
                            len(api.catalog_data) if hasattr(api, "catalog_data") else 0
                        ),
                        "encoders_loaded": (
                            hasattr(api.recommender, "encoders")
                            if hasattr(api, "recommender")
                            else False
                        ),
                        "api_version": "1.0.0",
                        "supported_genres": (
                            list(api.recommender.encoders.known_genres)
                            if hasattr(api, "recommender")
                            and hasattr(api.recommender, "encoders")
                            else []
                        ),
                    }
                )
