...
class RecommendationGenerator:
    def __init__(self, model_path: str, catalog_data: pd.DataFrame, encoders_path: str):
        ...
        try:
            ...
            self.encoders = encoder_dict.get(
                "encoder", None
            )  # This is now a DataEncoder object directly
    ...
    def generate_recommendations(
        self, user_info: dict, n_recommendations: int = 10
    ) -> pd.DataFrame:
        ...
        try:
            ...
            required_cols = self.encoders.numerical_features + [
                "music",
                "artist_name",
                "main_genre",
            ]
            missing_cols = [
                col for col in required_cols if col not in user_candidates.columns
            ]
            if missing_cols:
            ...
            test_dataset = MusicRecommenderDataset(
                user_candidates, mode="test", encoders=self.encoders
            )
        ...
        recommendations = recommendations.drop_duplicates(
            subset=["music"], keep="first"
        )

        # Convert predictions to scalar values and sort
        recommendations["predicted_plays"] = recommendations["predicted_plays"].apply(
            lambda x: float(x[0])
        )

        # Sort by predicted plays and get top N recommendations
        recommendations = recommendations.sort_values(
            "predicted_plays", ascending=False
        )
        ...
        try:
            ...
            recommendations["genre"] = recommendations["genre"].apply(
                lambda x: (
                    self.encoders.genre_encoder.inverse_transform([x])[0]
                    if str(x).isdigit()
                    else "Unknown"
                )
            )
...
class HybridMusicRecommender(nn.Module):
    def __init__(
        ...
        self.genre_embedding = nn.Embedding(
            genre_dims, embedding_dim
        )  # Keep this for categorical genre encoding
        ...
        self.binary_layer = nn.Sequential(
            nn.Linear(2, embedding_dim), nn.ReLU(), nn.BatchNorm1d(embedding_dim)
        )

        # Adjust total features to 5 embeddings * embedding_dim
        total_features = (
            embedding_dim * 5
        )  # music + artist + genre + numerical + binary
    ...
    def forward(self, batch):
        ...
        genre_emb = self.genre_embedding(
            batch["genre_features"]
        )  # Use embedding for categorical
        ...
        x = torch.cat([music_emb, artist_emb, genre_emb, numerical, binary], dim=1)
    ...
    def explain_prediction(self, batch):
        ...
        """
        return super().explain_prediction(
            batch
        )  # Utilize the method from train_model.py
...
def main():
    ...
    catalog_data = pd.read_csv(
        os.path.join(BASE_DIR, "data", "processed_data", "test_data.csv")
    )
    ...
    recommendations = recommender.generate_recommendations(
        user_info, n_recommendations=10
    )
